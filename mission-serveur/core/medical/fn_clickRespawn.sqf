...
if (_nbMedics > 0) then {
	...
	if (BANK < _mnt) exitWith {
		//["Banque", "Vous n'avez pas assez d'argent.", [0, 0, 0, 1], [0.607, 1, 0.341,1]] spawn Haz_fnc_createNotification;
	};
	BANK = BANK - _mnt;
	//["Banque", format ["Vous avez payé %1 pour ré-apparaître.", _mnt], [0, 0, 0, 1], [0.607, 1, 0.341,1]] spawn Haz_fnc_createNotification;
